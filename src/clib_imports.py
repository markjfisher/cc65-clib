...
# Jump-table ABI contract, shared with clib_stubs.py and clib_rom.cfg's JUMP_M.
JUMPTABLE_BASE = 0x8100
JUMPTABLE_SLOT_SIZE = 3


def jumptable_def_path() -> str:
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), "jumptable.def")


def read_jumptable_def() -> List[str]:
    """Return the ordered slot symbols from jumptable.def (slot index = list
    index). Blank lines and '#' comments are ignored. Missing file => no table."""
    path = jumptable_def_path()
    slots: List[str] = []
...
class InfoParser:
    ...
    def _write_jump_table(self, jumps_file: str, rom_exports: Dict[str, str]) -> None:
        """Write the JUMPTABLE-segment jump table from jumptable.def.

        Each non-RESERVED entry must be a ROM-resident symbol; we emit a 3-byte
        `jmp <symbol>`. RESERVED entries emit 3 zero bytes so later slots keep
        their fixed addresses. The segment is set by the includer (clib_rom.s).
        """
        slots = read_jumptable_def()
        with open(jumps_file, 'w') as f:
            f.write("; generated by clib_imports.py - CLIB ROM jump table\n")
            f.write("; slot order is fixed by jumptable.def (append-only)\n\n")
            for idx, sym in enumerate(slots):
                if sym == "RESERVED":
                    f.write(f"\t; slot {idx}: RESERVED\n")
    ...
    def generate_outputs(
        ...
        # Jump table (vectoring layer): one `jmp <function>` per slot, in the
        # permanent order given by jumptable.def, emitted into the JUMPTABLE
        # segment (placed at a fixed base by clib_rom.cfg). clib_stubs.py points
        # the listed symbols at these fixed slots so application binaries survive
        # ROM rebuilds.
        self._write_jump_table(jumps_file, my_exports_runtime)
